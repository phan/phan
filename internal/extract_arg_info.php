...
class OpcacheFuncInfoParser
{
    ...
    public static function main(): void
    {
        ...
        $php_src_version_file = $argv[1] . "/main/php_version.h";
        $php_src_version_contents = file_get_contents($php_src_version_file);
        if (
            $php_src_version_contents === false ||
            !preg_match('/#define PHP_VERSION "([\d.]+)"/', $php_src_version_contents, $matches)
        ) {
            fwrite(STDERR, "Can't find PHP version in $php_src_version_file\n");
            exit(1);
        }
        $php_src_version = $matches[1];
        $php_version = PHP_VERSION;
        $loaded_extensions_comment_lines = implode(
            "\n",
            /** @param string[] $exts */
            array_map(static function (array $exts): string {
               return ' *   ' . implode(', ', $exts) . ',';
            }, array_chunk(get_loaded_extensions(), 10))
        );
 ...
 * source version $php_src_version, using PHP version $php_version with extensions:
$loaded_extensions_comment_lines
 */
return [
$inner_contents
];
