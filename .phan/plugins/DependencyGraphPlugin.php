    ...
    FinalizeProcessCapability
{
    ...
    public function analyzeClass(CodeBase $unused_code_base, Clazz $class): void {
        ...
        if($class->isInterface()) $ctype = "I";
        else if($class->isTrait()) $ctype = "T";
        else $ctype = "C";
    ...
    private function walkcGraph(array $newgraph, string $node, int $depth = 0):array {
        ...
        if ($this->depth && $depth >= $this->depth) return $newgraph;
        if (!empty($visited[$node])) return $newgraph;
    ...
    private function walkfGraph(array $newgraph, string $node, int $depth = 0):array {
        ...
        if (!empty($visited[$node])) return $newgraph;
        CLI::progress('graph', (++$i) / count($this->cgraph), $node);
        if (!array_key_exists($node, $this->fgraph)) return $newgraph;
        ...
        foreach($graph as $k=>$v) {
            ...
            foreach($v as $kk=>$vv) {
                ...
                if (strstr((string)$kk, '.')) {
                    ...
                    if (!array_key_exists($kk, $this->class_to_file)) continue;
    ...
    public function finalizeProcess(CodeBase $unused_code_base): void {
        ...
        foreach ($this->elements as $element) {
            ...
            if($element->isInterface()) $ctype = "I";
            else if($element->isTrait()) $ctype = "T";
            else $ctype = "C";
        ...
        foreach (self::$static_calls as $c) {
            $cnode = key($c);
            if (!$cnode) continue;
            if (!array_key_exists($cnode, $this->class_to_file)) continue;
    ...
    private function dumpFileDot(string $title, array $graph = null):void {
        ...
        foreach($graph as $node=>$depNode) {
            ...
            foreach($depNode as $dnode=>$val) {
                ...
                $style = '';
                if($type=='s') $style = ',color=seagreen';
                if($type=='v') $style = ',color=tomato';
    ...
    private function dumpClassDot(string $title, array $graph = null):void {
        ...
        foreach($graph as $node=>$depNode) {
            ...
            if($shape) echo '"',addslashes(trim($node,"\\"))."\" [$shape]\n";
            foreach($depNode as $dnode=>$val) {
...
class DependencyGraphVisitor extends PluginAwarePostAnalysisVisitor {
    ...
    public function visitClassConst(Node $node):void {
        ...
        if ($context->isInGlobalScope()) return;
        $called_class = $node->children['class'];
        if (!isset($called_class->children['name'])) return;
        ...
        if ($called_class_name == 'self' || $called_class_name == 'parent' || $called_class_name == 'class' || $called_class_name == 'static') return;
