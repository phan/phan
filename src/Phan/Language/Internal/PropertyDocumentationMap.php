...
'CURLStringFile::data' => 'The contents to be uploaded.',
'CURLStringFile::mime' => 'MIME type of the file (default is `application/octet-stream`).',
'CURLStringFile::postname' => 'The name of the file to be used in the upload data.',
'DateInterval::d' => 'Number of days.',
'DateInterval::date_string' => 'The string used as argument to `DateInterval::createFromDateString`.',
'DateInterval::days' => 'If the DateInterval object was created by `DateTimeImmutable::diff` or `DateTime::diff`, then this is the total number of full days between the start and end dates. Otherwise, days will be `false`.',
'DateInterval::f' => 'Number of microseconds, as a fraction of a second.',
'DateInterval::from_string' => 'If the DateInterval object was created by `DateInterval::createFromDateString`, then this property\'s value will be `true`, and the date_string property will be populated. Otherwise, the value will be `false`, and the y to f, invert, and days properties will be populated.',
...
'DatePeriod::include_end_date' => 'Whether to include the end date in the set of recurring dates or not.',
...
'DatePeriod::recurrences' => 'The minimum amount of instances as retured by the iterator.
...
recurrences, "\n"; $period = new DatePeriod($start, $interval, $recurrences); echo $period->recurrences, "\n"; $period = new DatePeriod($start, $interval, $recurrences, DatePeriod::INCLUDE_END_DATE); echo $period->recurrences, "\n"; // recurrences not set in the constructor $period = new DatePeriod($start, $interval, $end); echo $period->recurrences, "\n"; $period = new DatePeriod($start, $interval, $end, DatePeriod::EXCLUDE_START_DATE); echo $period->recurrences, "\n"; ?>]]> `example.outputs` 5 6 7 1 0

See also `DatePeriod::getRecurrences`.',
'DatePeriod::start' => 'The start date of the period.',
'Deprecated::message' => 'An optional message explaining the reason for the deprecation and possible replacement functionality. Will be included in the emitted deprecation message.',
'Deprecated::since' => 'An optional string indicating since when the functionality is deprecated. The contents are not validated by PHP and may contain a version number, a date or any other value that is considered appropriate. Will be included in the emitted deprecation message.
...
'DOMNameSpaceNode::isConnected' => 'Whether the node is connected to a document.',
'DOMNameSpaceNode::localName' => 'The local part of the qualified name of this node.',
'DOMNameSpaceNode::namespaceURI' => 'The namespace URI declared by this node, or `null` if it is unspecified.',
'DOMNameSpaceNode::nodeName' => 'The qualified name of this node.',
'DOMNameSpaceNode::nodeType' => 'The type of the node. In this case it returns `XML_NAMESPACE_DECL_NODE` .',
'DOMNameSpaceNode::nodeValue' => 'The namespace URI declared by this node, or `null` if the empty namespace.',
'DOMNameSpaceNode::ownerDocument' => 'The `DOMDocument` object associated with this node, or `null` if this node is a `DOMDocument`',
'DOMNameSpaceNode::parentElement' => 'The parent element of this node. If there is no such element, this returns `null`.',
'DOMNameSpaceNode::parentNode' => 'The parent of this node. If there is no such node, this returns `null`.',
'DOMNameSpaceNode::prefix' => 'The namespace prefix declared by this node.',
...
'DOMNode::isConnected' => 'Whether the node is connected to a document',
...
'DOMNode::nodeType' => 'Gets the type of the node. One of the predefined `XML_*_NODE` constants',
...
'DOMNode::ownerDocument' => 'The `DOMDocument` object associated with this node, or `null` if this node does not have an associated document (e.g. if it is detached, or if it is a `DOMDocument`).',
'DOMNode::parentElement' => 'The parent element of this element. If there is no such element, this returns `null`.',
'DOMNode::parentNode' => 'The parent of this node. If there is no such node, this returns `null`.',
'DOMNode::prefix' => 'The namespace prefix of this node.',
...
'MongoDB\Driver\Exception\BulkWriteCommandException::errorReply' => 'Any top-level error that occurred when attempting to communicate with the server or execute the bulk write. This value may be `null` if the exception was thrown due to errors occurring on individual writes.',
'MongoDB\Driver\Exception\BulkWriteCommandException::partialResult' => 'A `MongoDB\Driver\BulkWriteCommandResult` reporting the result of any successful operations that were performed before the error was encountered. This value may be `null` if it cannot be determined that at least one write was successfully performed (and acknowledged).',
'MongoDB\Driver\Exception\BulkWriteCommandException::writeConcernErrors' => 'An array of any `MongoDB\Driver\WriteConcernError`s that occurred while executing the bulk write. This list may have multiple items if more than one server command was required to execute the bulk write.',
'MongoDB\Driver\Exception\BulkWriteCommandException::writeErrors' => 'An array of any `MongoDB\Driver\WriteError`s that occurred during the execution of individual write operations. Array keys will correspond to the index of the write operation from `MongoDB\Driver\BulkWriteCommand`. This map will contain at most one entry if the bulk write was ordered.',
'MongoDB\Driver\Exception\BulkWriteException::writeResult' => 'The `MongoDB\Driver\WriteResult` associated with the failed write operation.',
...
'mysqli_result::type' => 'Stores whether the result is buffered or unbuffered as an `integer` (`MYSQLI_STORE_RESULT` or `MYSQLI_USE_RESULT`, respectively).',
'mysqli_sql_exception::sqlstate' => 'The sql state with the error.',
'mysqli_stmt::id' => 'Stores the statement ID.',
...
'PDORow::queryString' => 'Query string used by the `PDOStatement` that returned the `PDORow` object.',
...
'PhpToken::id' => 'One of the T_* constants, or an ASCII codepoint representing a single-char token.',
'PhpToken::line' => 'The starting line number (1-based) of the token.',
'PhpToken::pos' => 'The starting position (0-based) in the tokenized string (the number of bytes).',
'PhpToken::text' => 'The textual content of the token.',
...
'Random\Randomizer::engine' => 'The low-level source of randomness for the `Random\Randomizer`’s methods.',
...
'RuntimeException::code' => '`SQLSTATE` error code. Use `Exception::getCode` to access it.',
'RuntimeException::errorInfo' => 'Corresponds to `PDO::errorInfo` or `PDOStatement::errorInfo`',
'RuntimeException::sqlstate' => 'The sql state with the error.',
'scalebarObj::position' => 'for embedded scalebars, MS_UL, MS_UC, ...',
'scalebarObj::status' => 'MS_ON, MS_OFF, MS_EMBED',
'SensitiveParameterValue::value' => 'The sensitive value to be protected against accidental exposure.',
...
'tidy::value' => 'The HTML representation of the node, including the surrounding tags.',
...
'ZipArchive::status' => 'Status of the Zip Archive. Available for closed archive, as of PHP 8.0.0 and PECL zip 1.18.0.',
'ZipArchive::statusSys' => 'System status of the Zip Archive. Available for closed archive, as of PHP 8.0.0 and PECL zip 1.18.0.',
];
