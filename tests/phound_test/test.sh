...
SIGNATURES=$(sqlite3 ~/phound.db 'SELECT fqsen, kind, class_fqsen, name, type, is_static, visibility, filepath, lineno FROM signatures ORDER BY fqsen')
PARAMETERS=$(sqlite3 ~/phound.db 'SELECT fqsen, idx, name, type, is_variadic, is_reference, is_optional, default_repr FROM parameters ORDER BY fqsen, idx')
