...
if [ ! -d expected  ]; then
	echo "Error: must run this script from tests/convergence_test folder" 1>&2
	exit 1
fi
...
../../phan --force-polyfill-parser --memory-limit 1G --analyze-until-convergence 2>/dev/null | tee $ACTUAL_PATH
