...
function decode_json_array(string $data) {
    ...
    $value2 = json_decode($data, null); // should warn: $associative is not nullable in PHP < 7.2
    if (is_array($value2)) {
        var_export($value2['key']);  // should warn (null was internally cast to false)
        var_export($value2[0]);
    }
...
function decode_json_object(string $data) {
    ...
    var_export($value3->fieldName);  // should not warn, the flag is ignored in PHP < 7.2
    $value4 = json_decode($data, null, 512, JSON_OBJECT_AS_ARRAY); // should warn, $assoc is not nullable in PHP < 7.2
    var_export($value4->fieldName);  // should not warn, null was internally cast to false
    $value5 = json_decode($data, false, 512, 0);
    var_export($value5->fieldName);  // should not warn
    $value6 = json_decode($data, null, 512, 0); // should warn, $assoc is not nullable in PHP < 7.2
